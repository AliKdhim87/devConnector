...
// Cancel the request from the arrays
const cancelFriendRequest = async (requestInfo) => {
  try {
    // Remove from the accepting user array
    await User.updateOne(
      { user: requestInfo.requestingUser.id },
      { $pull: { friendRequests: { user: requestInfo.friendRequest.id } } }
    );

    // Remove from the requesting user array
    await User.updateOne(
      { _id: requestInfo.acceptingUser.id },
      { $pull: { friendRequests: { _id: requestInfo.acceptingUser.id } } }
    );
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server error');
